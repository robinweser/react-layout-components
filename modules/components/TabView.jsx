  ...
  render() {
  	...
  	const selectedView = props.tabs.map((tab, index) => {
			if (index === this.state.selectedTab) {
				return <Box width="100%">{tab.view}</Box>
			}
		})

		const items = props.tabs.map((tab, index) => <Box center flex={1} onClick={() => {this.selectTab(index)}}>{tab.tab}</Box>)

    return (
			<Box {...props} flex={1} fit column>
				<Box fit>{selectedView}</Box>
				<Box flex="0 0 auto">{items}</Box>
			</Box>
		)
  }
}
